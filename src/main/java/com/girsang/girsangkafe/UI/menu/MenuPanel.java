...
import com.girsang.girsangkafe.util.BigDecimalRenderer;
import com.girsang.girsangkafe.util.Notifikasi;
import com.girsang.girsangkafe.util.UkuranTabel;
import com.girsang.girsangkafe.util.tabelmodel.TabelModelMenu;
...
//</editor-fold>
    ...
    private void tampilData(){
        txtCari.setText("");
        daftarMenu = GirsangKafe.getMasterService().semuaMenu();
        ...
        lblJumlah.setText(daftarMenu.size() +" Data Item   ");
    }
    ...
    private void baruBahanBaku(){
        ...
        if(m!=null){
            ...
            menu = new Menu();
            menu = m;
            List<Menu> daftarMenus = GirsangKafe.getMasterService().semuaMenu();
            for(int j=0;j<daftarMenus.size();j++){
                if(daftarMenus.get(j).getNamaMenu()
                        .equals(menu.getNamaMenu())&&
                        (daftarMenus.get(j).getId() == null ? menu.getId() 
                        != null : !daftarMenus.get(j).getId().equals(menu.getId()))){
            ...
            if(data==false){
                GirsangKafe.getMasterService().simpan(menu);
                tampilData();
            }
        }
    }
        ...
        if(menu==null){
               ...
               Menu m = new MenuDialog().showDialog(menu);
                if(m!=null){
                    boolean data = false;
                    menu = m;
                    List<Menu> bahanBakus = GirsangKafe.getMasterService().semuaMenu();
            for(int j=0;j<bahanBakus.size();j++){
                if(bahanBakus.get(j).getNamaMenu()
                        .equals(menu.getNamaMenu())&&
                        (bahanBakus.get(j).getId() == null ? menu.getId() 
                        != null : !bahanBakus.get(j).getId().equals(menu.getId()))){
                    tabel.setRowSelectionInterval(j, j);
                    Notifikasi.pesanDataSudahAda();
                    data = true;
                    }
    ...
    private void hapusMenu(){
        ...
        if(menu==null){
                ...
                if(Notifikasi.pesanValidasiHapus()){
                    GirsangKafe.getMasterService().hapus(menu);
                    tampilData();
                }
            }
    }
        ...
        txtCari.addKeyListener(new KeyListener() {
            ...
            public void keyReleased(KeyEvent ke) {
                if ("".equals(txtCari.getText())) {
                    ...
                    daftarMenu = GirsangKafe.getMasterService()
                            .menuBerdasaranNama(txtCari.getText());
                    tabel.setModel(new TabelModelMenu(daftarMenu));
                    tabel = new UkuranTabel().UkuranTabelMenu(tabel);
                }
            }
    ...
    private void initComponents() {
        ...
        lblJudul.setText("Daftar Menu");

        lblKeterangan.setFont(new java.awt.Font("Comic Sans MS", 1, 11)); // NOI18N
        lblKeterangan.setText("Disini anda bisa menambah, mengedit atau menghapus data menu");
