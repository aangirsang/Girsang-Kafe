...
import com.girsang.girsangkafe.util.tabelmodel.TabelModelKaryawan;
...
//</editor-fold>
    ...
    private void cariKaryawan(){
        if (!idSelect.equals("")) {
            ...
            karyawan = GirsangKafe.getMasterService().karyawanBerdasarkanId(idSelect);
        } else {
            karyawan = null;
        }
    }
    private void tampilData(){
        txtCari.setText("");
        daftarKaryawan = GirsangKafe.getMasterService().semuaKaryawan();
        tabel.setModel(new TabelModelKaryawan(daftarKaryawan));
        tabel = new UkuranTabel().UkuranTabelKaryawan(tabel);
        tabel.clearSelection();
        
        karyawan = null;
        idSelect = "";
        
        lblJumlah.setText(daftarKaryawan.size() +" Data Item   ");
    }
    
    private void baruKaryawan(){
        karyawan = null;
        Karyawan k = new KaryawanDialog().showDialog(karyawan);
        if(k!=null){
            boolean data = false;
            karyawan = new Karyawan();
            karyawan = k;
            List<Karyawan> karyawans = GirsangKafe.getMasterService().semuaKaryawan();
            for(int j=0;j<karyawans.size();j++){
                if((karyawans.get(j).getNamaLengkap()
                        .equals(karyawan.getNamaLengkap())) || 
                        karyawans.get(j).getNamaAkun()
                        .equals(karyawan.getNamaAkun()) &&
                        (karyawans.get(j).getId() == null ? karyawan.getId() 
                        != null : !karyawans.get(j).getId().equals(karyawan.getId()))){
            ...
            if(data==false){
                GirsangKafe.getMasterService().simpan(karyawan);
                tampilData();
            }
        }
    }
        ...
        if(karyawan==null){
               ...
               Karyawan k = new KaryawanDialog().showDialog(karyawan);
                if(k!=null){
                    ...
                    else if (karyawans.get(j).getNamaAkun()
                        .equals(karyawan.getNamaAkun()) &&
                            (karyawans.get(j).getId() == null ? karyawan.getId() 
                            != null : !karyawans.get(j).getId().equals(karyawan.getId()))){
                        tabel.setRowSelectionInterval(j, j);
                        Notifikasi.pesanAkunSudahAda();
                        data = true;
                    }
                    }
                    if(data==false){
    ...
    private void hapusKaryawan(){
        ...
        if(karyawan==null){
                ...
                if(Notifikasi.pesanValidasiHapus()){
                    GirsangKafe.getMasterService().hapus(karyawan);
                    tampilData();
                }
            }
    }
    private void addPopupmenu(){
        popup.add(new JMenuItem(new AbstractAction("Tambah Karyawan") {
            @Override
            public void actionPerformed(ActionEvent ae) {
                baruKaryawan();
            }
        }));
        popup.add(new JMenuItem(new AbstractAction("Edit Karyawan") {
            @Override
            public void actionPerformed(ActionEvent ae) {
                editKaryawan();
            }
        }));
        popup.add(new JMenuItem(new AbstractAction("Hapus Karyawawn") {
            @Override
            public void actionPerformed(ActionEvent ae) {
                hapusKaryawan();
            }
        ...
        txtCari.addKeyListener(new KeyListener() {
            ...
            public void keyReleased(KeyEvent ke) {
                if ("".equals(txtCari.getText())) {
                    ...
                    daftarKaryawan = GirsangKafe.getMasterService()
                            .karyawanBerdasaranNama(txtCari.getText());
                    tabel.setModel(new TabelModelKaryawan(daftarKaryawan));
                    tabel = new UkuranTabel().UkuranTabelKaryawan(tabel);
                }
            }
