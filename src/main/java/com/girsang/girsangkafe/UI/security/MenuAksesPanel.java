...
import com.girsang.girsangkafe.util.tabelmodel.TabelModelMenuAkses;
...
public class MenuAksesPanel extends javax.swing.JPanel {
...
//</editor-fold>
    public MenuAksesPanel() {
        initComponents();
        initListener();
        tampilData();
        addPopupmenu();
    }
    private void cariMenuAkses(){
        if (!idSelect.equals("")) {
            menuAkses = new MenuAkses();
            menuAkses = GirsangKafe.getMasterService().menuAksesBerdasarkanId(idSelect);
        } else {
            menuAkses = null;
        }
    }
    private void tampilData(){
        txtCari.setText("");
        daftarMenuAkses = GirsangKafe.getMasterService().semuaMenuAkses();
        tabel.setModel(new TabelModelMenuAkses(daftarMenuAkses));
        tabel = new UkuranTabel().UkuranTabelMenuAkses(tabel);
        tabel.clearSelection();
        
        menuAkses = null;
        idSelect = "";
        
        lblJumlah.setText(daftarMenuAkses.size() +" Data Item   ");
    }
    
    private void baruHakAkses(){
        menuAkses = null;
        MenuAkses mA = new MenuAksesDialog().showDialog(menuAkses);
        if(mA!=null){
            boolean data = false;
            menuAkses = new MenuAkses();
            menuAkses = mA;
            List<MenuAkses> menuAkseses = GirsangKafe.getMasterService().semuaMenuAkses();
            for(int j=0;j<menuAkseses.size();j++){
                if(menuAkseses.get(j).getMenuAkses()
                        .equals(menuAkses.getMenuAkses())&&
                        (menuAkseses.get(j).getId() == null ? menuAkses.getId() 
                        != null : !menuAkseses.get(j).getId().equals(menuAkses.getId()))){
            ...
            if(data==false){
                GirsangKafe.getMasterService().simpan(menuAkses);
                tampilData();
            }
        }
    }
    ...
    private void hapusHakAkses(){
        cariMenuAkses();
        if(menuAkses==null){
                Notifikasi.pesanTidakAdaData();
            }else{
                if(Notifikasi.pesanValidasiHapus()){
                    GirsangKafe.getMasterService().hapus(menuAkses);
                    tampilData();
                }
            }
    }
    private void addPopupmenu(){
        popup.add(new JMenuItem(new AbstractAction("Tambah Hak Akses") {
            @Override
            public void actionPerformed(ActionEvent ae) {
                baruHakAkses();
            }
        }));
        popup.add(new JMenuItem(new AbstractAction("Edit Hak Akses") {
            @Override
            public void actionPerformed(ActionEvent ae) {
                editHakAkses();
            }
        }));
        popup.add(new JMenuItem(new AbstractAction("Hapus Hak Akses") {
            @Override
            public void actionPerformed(ActionEvent ae) {
                hapusHakAkses();
            }
        ...
        txtCari.addKeyListener(new KeyListener() {
            ...
            public void keyReleased(KeyEvent ke) {
                if ("".equals(txtCari.getText())) {
                    ...
                    daftarMenuAkses = GirsangKafe.getMasterService()
                            .menuAksesBerdasaranNama(txtCari.getText());
                    tabel.setModel(new TabelModelMenuAkses(daftarMenuAkses));
                    tabel = new UkuranTabel().UkuranTabelMenuAkses(tabel);
                }
            }
    ...
    private void initComponents() {
        ...
        lblJudul.setText("Daftar Hak Akses");

        lblKeterangan.setFont(new java.awt.Font("Comic Sans MS", 1, 11)); // NOI18N
        lblKeterangan.setText("Disini anda bisa menambah, mengedit atau menghapus data hak akses");
